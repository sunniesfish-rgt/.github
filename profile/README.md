...
- **TypeScript**: 정적 타입을 통해 신뢰성과 유지보수성을 보장합니다.

### 백엔드

- **NestJS**: 모듈화된 Node.js 서버 프레임워크로 확장성과 구조화에 용이합니다.
- **MySQL**: 관계형 데이터베이스로 데이터 관리.
- **TypeORM**: 데이터베이스 작업을 단순화하는 ORM.
...
- **SEO 최적화**:
  - OpenGraph와 메타데이터 설정.
  - sitemap.xml 및 robots.txt로 검색 엔진 최적화.
- **데이터 관리**:
  - React Query와 Zustand를 사용한 효율적인 데이터 및 상태 관리.
  - 실시간 검색과 필터링 기능.
- **UX 최적화**:
  - 서버 사이드 렌더링과 코드 스플리팅.
  - Suspense로 로딩 중 사용자 경험 개선.
...
1. **서버 사이드 데이터 페칭 (RSC)**
   - React Server Components를 사용하여 서버에서 직접 데이터를 가져옵니다.
   - 클라이언트 사이드에서 불필요한 데이터 페칭을 줄이고, 초기 로딩 시간을 단축시킵니다.
   - 서버에서 데이터를 렌더링하므로 SEO 최적화에 유리합니다.

2. **클라이언트 사이드 데이터 관리 (TanStack Query)**
   - RSC를 통해 받은 초기 데이터를 TanStack Query의 `initialData`로 설정하여, 클라이언트에서 즉시 사용할 수 있도록 합니다.
   - 적절한 `staleTime`을 설정하여 데이터가 일정 시간 동안 신선하게 유지되도록 하여, 불필요한 재요청을 방지합니다.
   - 데이터 변경 시 상황에 맞는 `invalidateQueries`를 사용하여, 필요한 경우에만 데이터를 갱신하여 성능을 최적화합니다.

3. **사용자 경험 개선**
   - 초기 로딩 시 서버에서 데이터를 가져와 사용자에게 빠르게 콘텐츠를 제공합니다.
   - 클라이언트에서 데이터가 변경되면 TanStack Query가 자동으로 업데이트를 관리하여, 사용자에게 최신 정보를 제공합니다.
